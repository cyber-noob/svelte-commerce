	...
	<div
		...
		<a
			...
			{#if product_image_dimension == '1x1'}
				{#if product?.product?.general_info.photos}
					<LazyImg
						src="{product?.product?.general_info.photos[0].url}"
						alt="{product?.product?.general_info.title}"
						height="210"
						aspect_ratio="1:1"
				...
				<a
					...
					<div class="flex gap-2 justify-between">
						<h4
							...
							{product?.product?.general_info.title || '_'}
						</h4>
      ...
      {#if product?.product?.general_info?.collection}
        <h3 class="text-xs sm:text-sm text-zinc-500 truncate">
          {product?.product?.general_info?.collection}
        </h3>
      {/if}

			{#if product?.product?.general_info?.price}
				<a
					href="/product/{product?.product?.slug}"
					aria-label="Click to view the product details"
					data-sveltekit-preload-data>
					...
					{#if product?.product?.general_info?.price}
						<div
							...
							<span class="text-base font-bold whitespace-nowrap leading-3">
								{currency(product?.product?.general_info.price, product?.product?.general_info?.currency_symbol)}
							</span>

							{#if product?.product?.general_info?.mrp > product?.product?.general_info?.price}
								<span class="text-zinc-500 line-through whitespace-nowrap">
									{currency(product?.product?.general_info.mrp, product?.product?.general_info?.currency_symbol)}
								</span>

								{#if Math.floor(((product?.product?.general_info?.mrp - product?.product?.general_info?.price) / product?.product?.general_info?.mrp) * 100) > 0 }
									<span class="text-secondary-500 whitespace-nowrap">
										({Math.floor(((product?.product?.general_info?.mrp - product?.product?.general_info?.price) / product?.product?.general_info?.mrp) * 100)}% off)
									</span>
								{/if}
							{/if}
						</div>
