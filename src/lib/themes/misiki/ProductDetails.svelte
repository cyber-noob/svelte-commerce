...
<div class="min-h-screen lg:p-10">
  <div class="md:container md:mx-auto flex flex-col gap-5">
            ...
            {#if showLongDescription}
              <div transition:slide="{{ duration: 300 }}" class="pb-5 prose max-w-none">
                {#if data.product.categoryPool[0].name === 'Pet_Collection'}
                  <Puppy product={data.product} />
                {:else if data.product.categoryPool[0].name === 'Pet_Accessories'}
                  {#if data.product.categoryPool[2].name === 'ChewToy'}
                    <ChewToy product={data.product} />
                  {/if}
                {/if}
              </div>
            {/if}
    ...
    <BookAVideoCall show={display} seller={data?.product?.general_info?.seller_id}
                    product={data?.product?.general_info?.uuid} data={data} />
